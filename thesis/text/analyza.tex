...
Táto diplomová práca sa zameriava na vytvorenie webovej verzie súčasnej aplikácie vrátane zarovnania mriežok pomocou \texttt{grid-tracker} podprogramu \ref{podprogramy}.
...
\subsection {Používateľské rozhranie}\label{stare_ui}
...
\subsubsection {Spracovanie a zobrazenie MR snímkov}
Do aplikácie by malo byť možné nahrať snímky z magnetickej rezonancie vo formáte DICOM a tieto snímky taktiež zobraziť.

\subsubsection {Animácia MR snímkov}
Aplikácia by mala umožniť animovať nahrané snímky pre jednoduchšiu analýzu pohybu myokardu. Parametre animácie ako jej rýchlosť a výber fotky, od/do ktorej snímky má animácia prebiehať by mali byť upraviteľné.

\subsubsection {Zobrazenie a interaktívna úprava mriežky}
Implementovaná aplikácia by mala vedieť zobraziť mriežku nad snímkou z MR. Táto mriežka by mala byť interaktívna, t.j. polohu jej bodov by bolo možné interaktívne upravovať. Parametre tejto mriežky (\ref{stare_ui}) by sa taktiež mali dať upraviť podľa želania používateľa.

\subsubsection {Zadanie parametrov pre \texttt{grid-tracker} podprogram}
Pre korektné spustenie \texttt{grid-tracker} podprogramu je nutné tomuto podprogramu podsunúť rozličné parametre. Tieto parametre by sa mali dať definovať v aplikácií pre ich neskoršie použitie v \texttt{grid-tracker} podprograme. Výpis týchto parametrov je možné nájsť v \ref{podprogramy}.

\subsubsection {Spustenie \texttt{grid-tracker} podprogramu a zobrazenie jeho výsledkov}
Aplikácia by mala umožniť spustiť \texttt{grid-tracker} podprogram, ktorý zarovná mriežku definovanú používateľom s mriežkou, ktorá bola vygenerovaná pomocou techniky SPAMM. Po jej zarovnaní by mala byť aplikácia schopná zobraziť upravenú mriežku.
...
Prvou nefunkčnou požiadavkou je vytvorenie webovej aplikácie, ktorá by mala byť prístupná zo všetkých moderných webových prehliadačov. Pre lekárov výber tejto architektúry zjednoduší jej prístupnosť, nakoľko k takejto aplikácii môže pristupovať z rôznych zariadení a platforiem bez nutnosti inštalácie aplikácie a jej následnej podpory na týchto zariadeniach.
...
Práca s osobnými dátami by mala byť do maximálnej možnej miere naprieč aplikáciou minimalizovaná, aby sa predišlo únikom citlivých údajov o pacientovi.
