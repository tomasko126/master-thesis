...
Spustenie podprogramu a zobrazenie jeho výsledku (súradnice bodov mriežok) vyžaduje nielen mať importované DICOM snímky v aplikácii, ale aj vytvorenú mriežku s upravenými parametrami a zadanými parametrami pre \texttt{grid-tracker} podprogram. To je dôvodom, prečo tento scenár použitia zahŕňa prípady \uv{\nameref{uc1}}, \uv{\nameref{uc3}}, \uv{\nameref{uc4}} a \uv{\nameref{uc5}}. Samotný scenár realizuje funkčnú požiadavku \uv{\nameref{fr5}}.
...
Tieto frameworky (a iné aplikácie postavené nad Node.js) sú poskytované formou balíčkov. Tieto balíčky zvyknú byť dostupné v oficiálnom manažérovi balíčkov pre Node.js -- npm. Pomocou npm je možné nainštalovať (globálne i lokálne) rôzne balíčky, ktoré môžu byť použité pri vývoji aplikácií. Pre inštaláciu balíčka stačí exekuovať príkaz \texttt{npm install packagename}. Okrem inštalácií balíčkov je možné tieto balíčky spravovať, aktualizovať na novšie verzie či odinštalovať, a iné. Počas inštalácie Node.js je automaticky nainštalovaný aj tento manažér balíčkov.
...
\subsection {Nuxt.js}
Nuxt.js je voľne dostupný open-source framework, pomocou ktorého je možné vytvárať full-stack webové aplikácie a stránky pomocou Vue.js\footnote{https://vuejs.org}. Vue.js technológia bude popísaná v samostatnej podsekcii nižšie.

Nuxt.js ponúka automatický routing na základe štruktúry súborov v \texttt{/pages} zložke. Taktiež automaticky delí kód na menšie celky, čo môže pomôcť s prvým načítaním webovej aplikácie. Okrem renderovania obsahu až na klientovi, je možné renderovať obsah už na serveri a takýto obsah poslať naspäť webovému prehliadaču. Pomocou automatických importov Vue.js komponentov nie je potrebné explicitne importovať použité Vue.js komponenty. Okrem toho nie len podporuje, ale aj celý framework je naprogramovaný v TypeScripte, čo znamená že je možné využívať type-hinty čo sa týka funkcionality Nuxt.js pri programovaní webovej aplikácie i bez nutnosti použitia TypeScriptu.

Na pozadí používa Nuxt.js Nitro\footnote{https://nitro.unjs.io/} server, ktorý generuje API endpointy na základe štruktúry súborov nachádzajúcich sa v zložke \texttt{server/api}. Nitro je taktiež zodpovedné za zostavenie full-stack aplikácie -- výstupom zostavenia je \texttt{.output} zložka obsahujúca minifikované súbory zbavené všetkých nepoužitých závislostí. Spustenie takto zostavenej full-stack aplikácie je možné následne jediným príkazom: \texttt{node .output/index.mjs}. \texttt{.output} zložku je následne možné nasadiť na server podporujúci Node.js.
...
Nižšie je uvedený príklad využitia Vue.js frameworku -- pomocou vytvorenia tzv. Single File Componentu (SFC), ktorý v rámci jedného súboru kombinuje použitie HTML, CSS a JS ako v popise uvádzanom vyššie.
...
\subsection {Next.js}
Next.js je podobným frameworkom ako Nuxt.js s tým rozdielom, že nepoužíva Vue.js pre vytváranie znovupoužiteľných UI komponentov, ale iný framework -- React.
...
\section {Návrh komunikácie webového rozhrania so serverom}
